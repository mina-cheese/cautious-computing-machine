...
note: tested some small cluster prime values and they all worked. Should write a loop to compile a table
...
```{r}
for (n in 1:100) {
  next_cp <- find_next_cluster_prime(n, primes, diff, odd_comp)
  
  print(c(next_cp, primes[next_cp]))
}
```

We can also use this algorithm to tabulate the distribution of cluster primes. 
...
nothing cuz ur an idot womp womp
